...
module.exports = {
  ...
  execute: async (client, message, args, embed, author, channel, guild) => {
    ...
    message.reply({ content: (`${member} kullanıcısı **"${reason}"** sebebiyle **${sure}** boyunca sesli kanallarda susturuldu. \`(Ceza ID: #${db.fetch(`ceza_${guild.id}`)})\``)}).catch((err) => console.log(err), client.ytick(message))
