...
module.exports = {
  ...
  execute: async (client, message, args, embed, author, channel, guild) => {
    ...
    .setDescription(`**${user}** - **(${user.id})** kullanıcısı ${author} tarafından **"${reason}"** sebebiyle sunucudan yasaklandı. \`(Ceza ID: #${db.fetch(`ceza_${guild.id}`)})\``)

    message.reply({ embeds: [messageEmbed]});
    if (config.bot.dmMessages) user.send(`**${message.guild.name}** sunucumuzda, **${message.author.tag}** yetkilisi tarafından yasaklandın.`).catch(() => {});
