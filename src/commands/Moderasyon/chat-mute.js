...
module.exports = {
  ...
  execute: async (client, message, args, embed, author, channel, guild) => {
    if (!message.member.permissions.has("ADMINISTRATOR") && !message.member.roles.cache.has(config.penals.mute.staff)) return message.reply({ embeds: [embed.setDescription(`Bu komutu kullanabilmek için öncelikle gerekli yetkin olmalı!`)] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
    ...
    message.reply({ content: (`${member} kullanıcısı **"${reason}"** sebebiyle **${sure}** boyunca susturuldu. \`(Ceza ID: #${db.fetch(`ceza_${guild.id}`)})\``)}).catch((err) => console.log(err), client.ytick(message))
    member.roles.add(config.penals.mute.roles)
    db.set(`cikgirmute_${member.user.id}`, true)
    ...
    setTimeout(() => {
      ...
      client.channels.cache.get(config.penals.mute.log).send({ embeds: [embed.setDescription(`${member} kişisinin **${moment(Date.now()).format("LLL")}** tarihinde **${reason}** sebebiyle aldığı susturulma cezası süresi bittiği için kaldırıldı.`)] })}
    }, ms(sure));
    db.set(`cikgirmute_${member.user.id}`, false)
