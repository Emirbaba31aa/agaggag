...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        message.reply({ content: (`${member} kullanıcısı **${reason}** sebebiyle karantinaya atıldı. \`(Ceza ID: #${db.fetch(`ceza_${guild.id}`)})\``)}).catch((err) => console.log(err), client.ytick(message))
        db.add(`ceza_${guild.id}`, 1)
        db.set(`cikgirjail_${member.user.id}`, true)
