...
module.exports = {
    ...
    if (Member.voice.channel === message.member.voice.channel) return message.reply({ embeds: [embed.setDescription(`Şu anda zaten aynı kanaldasınız.`)] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
    ...
    if (!message.member.roles.cache.has(config.penals.transport.staff)  === true) {
        ...
        message.reply({ embeds: [embed.setDescription(`${Member} kullanıcısı başarıyla yanınıza taşındı.`)] }).catch((err) => console.log(err), client.ytick(message))
    } else {
