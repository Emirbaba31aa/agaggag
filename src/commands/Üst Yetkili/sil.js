...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        if (!args[0]) return message.reply({ embeds: [embed.setDescription("Öncelikle 1-100 arasında bir rakam belirtiniz.")] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
        if (isNaN(args[0])) return message.reply({ embeds: [embed.setDescription("Öncelikle geçerli bir sayı belirt!")] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
