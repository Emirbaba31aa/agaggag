...
module.exports = {
  ...
  execute: async (client, message, args, embed, author, channel, guild) => {
    ...
    if (!member) return message.reply({ embeds: [embed.setDescription("Öncelikle rol geçmişine bakacağın kullanıcıyı belirtmelisin!")] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
    const role = db.get(`rolelog_${member.id}`);
    if (!role) return message.reply({ embeds: [embed.setDescription(`${member} kullanıcısının daha önceden kayıtlı herhangi bir rol verisi bulunmamaktadır.`)] }).catch((err) => console.log(err), client.ytick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
