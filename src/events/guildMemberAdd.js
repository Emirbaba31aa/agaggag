...
module.exports = async member => {
    ...
    if (db.get(`cikgirmute_${member.user.id}`)) member.roles.add(config.penals.mute.roles)
    if (db.get(`cikgirjail_${member.user.id}`)) member.roles.add(config.penals.jail.roles)
    if (kurulus > 604800000) {
        member.roles.add(config.registration.unregistered);
        member.roles.add(config.registration.unregistered);
        member.setNickname(config.registration.autonickname);
        member.guild.channels.cache.get(config.channels.welcomechannel).send(`**${config.Guild.GuilDName}** sunucumuza hoş geldin, ${member} ${config.emojis.tada}
        
Hesabın **${kuruluş}** tarihinde (\`${zaman}\`) önce oluşturulmuş.        
Seninle birlikte **${member.guild.memberCount}** kişi olduk!
Sol tarafta bulunan " **__V. Confirmed__** " odalarından birine girerek kayıt işlemini gerçekleştirebilirsin!
        
Taglarımızdan birini **(.tag)** alarak ailemize katılabilirsin`);
    } else {
        member.setNickname(config.registration.susoeciosnickname);
        member.roles.add(config.registration.suspecios);
        member.setNickname(config.registration.autonickname);
        member.guild.channels.cache.get(config.channels.welcomechannel).send(`${member}, kullanıcısı sunucumuza katıldı. 
        
Hesabı **${zaman2}** önce açıldığı için şüpheli olarak işaretlendi.`);
    }
}
