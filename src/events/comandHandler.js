...
module.exports = async (message) => {
    if (!message.guild || message.author.bot) return
      const owner = client.users.cache.get("986415932421337119");
    if (["tag", "TAG", "Tag", ".tag", "!tag"].some(x => message.content === x)) {
        message.reply({ content:`\`${config.registration.GuilDTag}\`, \`${config.registration.GuilDTag2}\`, \`#${config.registration.GuildDiscrim}\``}) 
    }
...
if (message.author.id === uye) {
    ...
    const borangkdn = client.users.cache.get("986415932421337119");
